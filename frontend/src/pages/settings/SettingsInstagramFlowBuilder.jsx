...
const NODE_TYPES = [
  ...
  // OPEN_META_FLOW (encrypted Meta WhatsApp Flows) is intentionally omitted — Instagram
  // has no Meta Flows API, so those forms cannot run over Instagram DMs.
...
function buildSearchEnquiryTemplate() {
  return {
    ...
    nodes: [
      ...
      { id: 'pk_save', type: 'SAVE_ENQUIRY', position: { x: 1620, y: 60 }, data: { status: 'ENQUIRY', notePrefix: 'Package enquiry', finalMessage: 'Thank you. Our team will contact you shortly.' } },
      ...
      { id: 'pr_save', type: 'SAVE_ENQUIRY', position: { x: 1800, y: 300 }, data: { status: 'ENQUIRY', notePrefix: 'Property enquiry', finalMessage: 'Thank you. Our team will contact you shortly.' } },
      ...
      { id: 'sv_save', type: 'SAVE_ENQUIRY', position: { x: 1360, y: 560 }, data: { status: 'ENQUIRY', notePrefix: 'Service enquiry', finalMessage: 'Thank you. Our team will contact you shortly.' } },
...
function flowLibraryFromAgencyConfig(config) {
  if (Number(config?.schemaVersion) >= 4 && Array.isArray(config.flows)) {
    ...
    const merged = [
      ...flows,
      ...DEFAULT_DOMAIN_FLOWS.filter((template) => !flows.some((flow) => flow.id === template.id)),
    ];
    return {
  ...
  return {
    ...
    flows: [entryFlow, ...DEFAULT_DOMAIN_FLOWS],
  };
}
...
function getDefaultNodeData(type) {
  const defaults = {
    ...
    WHATSAPP_BUTTON: {
      body: 'Our travel consultant will contact you shortly. Tap below to chat with our staff on WhatsApp.',
      buttonLabel: 'Chat on WhatsApp',
      target: 'ASSIGNED_AGENT',
      phone: '',
    },
    SEND_ITEM_DETAIL: { catalogType: 'AUTO', message: '' },
    SEND_ITEM_DOCUMENT: { catalogType: 'PACKAGE', fallbackMessage: 'The PDF is not available yet. Our team will share it shortly.' },
    ...
    HANDOFF: { reason: 'Requested from Instagram flow builder' },
...
function validateGraph(nodes, edges) {
  ...
  nodes.forEach((node) => {
    ...
    if (!OPTION_NODE_TYPES.has(node.type) && !['END', 'HANDOFF', 'SAVE_ENQUIRY', 'SEND_ITEM_DETAIL', 'SEND_ITEM_DOCUMENT', 'WHATSAPP_BUTTON', 'OPEN_SERVICE', 'OPEN_PACKAGE_FLOW', 'OPEN_PROPERTY_FLOW', 'OPEN_VISA_FLOW', 'OPEN_CRUISE_FLOW', 'OPEN_SERVICE_FLOW', 'OPEN_CUSTOM_TRIP_FLOW', 'OPEN_META_FLOW'].includes(node.type)) {
    ...
    <div className="space-y-4 rounded-lg border border-neutral-200 bg-white p-4">
      ...
      {selectedNode.type === 'CATALOG_LIST' ? (
        <>
          ...
          <Field label="Instagram display">
            <select value={data.igCardMode || 'LIST'} onChange={(event) => update({ igCardMode: event.target.value })} className="shell-input-rect bg-white text-sm">
              <option value="LIST">List — numbered text + images</option>
              <option value="CARDS">Cards — one card per item (image + text)</option>
              <option value="CAROUSEL">Carousel — swipeable cards</option>
            </select>
          </Field>
...
function Preview({ nodes, edges, agency, properties = [], flows = [], catalogItems = {} }) {
  ...
  const agencyName = agency?.name || 'Instagram CRM';
...
export default function SettingsInstagramFlowBuilder({ fullScreen = false }) {
  const { agency, updateAgency } = useAuthStore();
  const initialLibrary = useMemo(() => flowLibraryFromAgencyConfig(agency?.instagramFlowConfig || {}), [agency?.instagramFlowConfig]);
  ...
  const [activeFlowId, setActiveFlowId] = useState(initialLibrary.entryFlowId);
  const initialActiveFlow = initialLibrary.flows.find((flow) => flow.id === initialLibrary.entryFlowId) || initialLibrary.flows[0] || graphToEditableFlow(DEFAULT_GRAPH, 'entry', 'Entry Menu');
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: ({ data: response }) => {
      ...
      toast.success('Instagram flow builder saved');
    },
  ...
  return (
    <>
      ...
      <div className={`md:hidden ${fullScreen ? "-m-3 min-h-[calc(100dvh-4rem)] bg-[#f5f5f5] p-3 sm:-m-4 sm:p-4" : ""}`}>
        <div className="mx-auto flex max-w-sm flex-col items-center gap-3 rounded-lg border border-neutral-200 bg-white px-6 py-10 text-center">
          ...
          <p className="text-sm text-neutral-500">
            The Instagram Flow Builder is a drag-and-drop canvas designed for laptops and desktops. Please open this page on a wider screen to edit your flow.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-extrabold text-neutral-950">Instagram Flow Builder</h2>
          <p className="mt-1 text-sm text-neutral-500">Design the customer conversation that starts when someone messages your Instagram account in a DM.</p>
