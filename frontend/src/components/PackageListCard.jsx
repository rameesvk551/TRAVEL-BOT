...
import { MapPinIcon, ArrowUpRightIcon, PencilSquareIcon, EyeIcon, EyeSlashIcon } from '@heroicons/react/24/outline';

export default function PackageListCard({ pkg, onEdit, onToggleActive, canManage, onClick }) {
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-0.5 py-2.5 px-3 md:p-4">
        ...
        <div className="flex items-center gap-1 text-neutral-500">
          ...
          <span className="truncate text-[11px] font-medium">{pkg.destinations?.join(', ') || 'Various'}</span>
          {pkg.duration && (
            <>
              <span className="mx-1 text-neutral-200">·</span>
              <span className="text-[11px] font-medium text-neutral-400 flex-shrink-0">{pkg.duration}</span>
            </>
          )}
        </div>
