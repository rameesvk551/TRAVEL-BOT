...
export function createMessageService(
    ...
    async function sendTemplate(dto: any) {
        try {
            const context = await conversationService.getOrCreateContext(dto.tenantId, dto.channel ?? 'WHATSAPP', dto.recipientPhone, 'SALES_AGENT');
            const adapter = channelFactory.getAdapter(context.channel, dto.tenantId);
            const runtimeComponents = Array.isArray(dto.components) ? dto.components : [];
            const templateVariables = dto.variables && typeof dto.variables === 'object' && !Array.isArray(dto.variables)
                ? dto.variables
                : {};

            let templateComponents = runtimeComponents;

            if (templateComponents.length === 0) {
                const pool = getPool();
                let templateResult = await pool.query(
                    `SELECT components FROM whatsapp_templates WHERE template_name = $1 AND tenant_id::text = $2 ORDER BY updated_at DESC LIMIT 1`,
                    [dto.templateName, dto.tenantId]
                );

                if (!templateResult.rows?.length) {
                    const mongoTenant = await TenantModel.findOne({ slug: dto.tenantId }).select('email').lean();
                    if (mongoTenant?.email) {
                        const sqlTenant = await pool.query(
                            'SELECT tenant_id FROM users WHERE email = $1 ORDER BY created_at ASC LIMIT 1',
                            [mongoTenant.email]
                        );
                        const resolvedTenantId = sqlTenant.rows?.[0]?.tenant_id;
                        if (resolvedTenantId) {
                            templateResult = await pool.query(
                                `SELECT components FROM whatsapp_templates WHERE template_name = $1 AND tenant_id::text = $2 ORDER BY updated_at DESC LIMIT 1`,
                                [dto.templateName, resolvedTenantId]
                            );
                        }
                    }
                }

                const rawTemplateComponents = templateResult.rows?.[0]?.components;
                templateComponents = Array.isArray(rawTemplateComponents)
                    ? rawTemplateComponents
                    : (typeof rawTemplateComponents === 'string' ? JSON.parse(rawTemplateComponents) : []);
            }

            const providerMessageId = await adapter.sendTemplate(context, dto.templateName, dto.language || 'en', templateVariables, templateComponents);
            const message = WhatsAppMessage.create({
                tenantId: dto.tenantId, conversationId: context.id, providerMessageId, providerTimestamp: new Date(),
                direction: 'OUTBOUND', senderPhone: dto.recipientPhone, recipientPhone: dto.recipientPhone,
                messageType: 'TEMPLATE', templateContent: { templateName: dto.templateName, language: dto.language || 'en', components: templateComponents },
    ...
    async function sendMedia(dto: any) {
        try {
            const context = await conversationService.getOrCreateContext(dto.tenantId, dto.channel ?? 'WHATSAPP', dto.recipientPhone, 'SALES_AGENT');
            const adapter = channelFactory.getAdapter(context.channel, dto.tenantId);
            const providerMessageId = await adapter.sendMedia(
                context,
                dto.mediaUrl,
                dto.caption,
                dto.mediaType || 'image'
            ...
            const message = WhatsAppMessage.create({
                ...
                recipientPhone: dto.recipientPhone,
                messageType: String(dto.mediaType || 'IMAGE').toUpperCase() as any,
                mediaContent: {
                ...
                handledByUserId: sanitizeUserId(dto.senderUserId),
                isProcessed: true,
            ...
            emitEvent(dto.tenantId, context.id, 'whatsapp:message', {
                ...
                conversationId: context.id,
                direction: 'OUTBOUND',
                type: String(dto.mediaType || 'IMAGE').toUpperCase(),
                content: {
                    caption: dto.caption,
            ...
            emitEvent(dto.tenantId, context.id, 'whatsapp:conversation_updated', {
                conversationId: context.id,
                lastMessagePreview: dto.caption || 'Sent an image',
                lastMessageAt: new Date().toISOString(),
            });

            return { success: true, messageId: saved.id, providerMessageId };
        } catch (error: any) {
            console.error('[MessageService] sendMedia failed:', error.message);
            return { success: false, error: error.message };
        }
    }
        ...
        try {
            ...
            if (!dto.interactiveContent && dto.bodyText) {
                dto.interactiveContent = {
                    ...
                    buttons: (dto.buttons || []).map((b: any) => ({ 
                        id: b.id, 
                        title: (b.title || 'Select').trim() || 'Select' 
                    })),
                };
            }
    ...
    return { processInbound, getMessagesByConversation, sendText, sendTemplate, sendMedia, sendInteractive, handleStatusUpdate, getSuggestedActions, requiresHuman, generatePaymentLink };
}
