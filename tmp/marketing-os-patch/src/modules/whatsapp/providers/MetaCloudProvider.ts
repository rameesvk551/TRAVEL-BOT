...
export function createMetaCloudProvider(config: MetaConfig): IWhatsAppProvider {
  ...
  async function sendTemplate(
    ...
    try {
      const normalizeParameter = (param: any) => {
        const type = String(param?.type || '').toLowerCase();
        const value = param?.value ?? param?.text ?? null;
        const p: any = { type };

        if (type === 'text') {
          p.text = value ?? '';
        } else if (type === 'image') {
          p.image = { link: param?.image?.link || value };
        } else if (type === 'video') {
          p.video = { link: param?.video?.link || value };
        } else if (type === 'document') {
          p.document = {
            link: param?.document?.link || value,
            ...(param?.document?.filename || param?.filename ? { filename: param?.document?.filename || param?.filename } : {}),
          };
        }
        ...
        template: {
          ...
          components: components.map(comp => {
            if (comp.type === 'carousel') {
              return {
                type: 'carousel',
                cards: comp.cards.map((card: any) => ({
                  card_index: card.cardIndex,
                  components: card.components.map((cardComp: any) => ({
                    type: cardComp.type,
                    parameters: cardComp.parameters.map((param: any) => normalizeParameter(param))
                  }))
                }))
              };
            }
            return {
              type: comp.type,
              parameters: comp.parameters.map((param: any) => normalizeParameter(param)),
            };
          }),
        },
      };
