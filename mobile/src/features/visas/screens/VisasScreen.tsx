...
export function VisasScreen({ navigation }: any) {
  ...
  const { data: visas, isLoading, refetch } = useVisas(statuses[status]);
    ...
    return (
      <Card style={{ padding: s.s3, marginBottom: s.s4 }}>
          ...
          <View style={{ flex: 1, flexDirection: 'row', gap: s.s3 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4, gap: s.s2 }}>
                ...
                <Text style={[theme.typography.caption2, { color: theme.colors.text.secondary }]}>{item.processingTime}</Text>
              </View>
            </View>
          </View>
        </View>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: s.s3 }}>
          ...
          <Button variant="tinted" label="Edit" onPress={() => navigation.navigate('VisaForm', { visaId: item.id })} />
        </View>
      </Card>
    );
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg.canvas }} edges={['top']}>
      <View style={{ paddingHorizontal: s.s4, paddingTop: s.s4, paddingBottom: s.s4 }}>
        <Text style={[theme.typography.largeTitle, { color: theme.colors.text.primary, marginBottom: s.s4 }]}>Visas</Text>
      ...
      <View style={{ flex: 1, paddingHorizontal: s.s4 }}>
        {isLoading ? (
          ...
          <EmptyState icon={FileText} title="No visas found" message="Try adjusting your filters or create a new one." />
        ) : (
          <FL
            data={visas}
      ...
      <View style={{ position: 'absolute', bottom: s.s6, right: s.s4 }}>
        <Button variant="fab" icon={Plus} onPress={() => navigation.navigate('VisaForm')} />
      </View>
    </SafeAreaView>
  );
}
